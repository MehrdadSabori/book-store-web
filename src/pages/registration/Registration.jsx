...
function Registration() {
  ...
  const submitHandeler = async (e) => {
    ...
    await fetch('https://parseapi.back4app.com/users', {
      method: 'POST',
      headers: {
      ...
      .then((res) => {
        if (res.status == 201) {
          ...
          ShowAlert('warning', 'شما قبلا ثبت نام کرده اید', contextDatas.darkMode && true);
        } else {
          ShowAlert('error', 'عدم ارتباط با سرور', contextDatas.darkMode && true)
        }
      })
      .then(data => {
        if (data !== undefined) {
          authContext.logIn(data, data.sessionToken);
          ShowAlert('success', 'شما با موفقیت ثبت نام شدید', contextDatas.darkMode && true);
  ...
  const focusHandeler = (e) => {
    if (e.target.name === 'confrimPassword') {
      document.querySelector('input[name="confrimPassword"]').setAttribute('pattern', values.password);
    }
  }
  return (
    <div className='registration'>
      <div className="register_contaner">
        <form className="form_section" onSubmit={submitHandeler}>
          <span className='form_header'>ثبت نام کنید</span>
          ...
          <span className="btn spanLink" onClick={() => navigate('/login')}>ورود به پنل کاربری</span>
          <input type='submit' value={'ثبت نام '} className="btn submit_btn" />
        </form>
